...
class DictTrie {
 public:
  ...
  bool IsUserDictSingleChineseWord(const Rune& word) const {
    return user_dict_single_chinese_word_.end() != std::find(user_dict_single_chinese_word_.begin(),
  ...
  void InserUserDictNode(const string& line) {
    vector<string> buf = hola::Split(line, " ");
    DictUnit node_info;
    if (buf.size() == 1) {
  ...
  void LoadUserDict(const string& filePaths) {
    vector<string> files = hola::Split(filePaths, "|;");
    size_t lineno = 0;
 ...
 private:
  ...
  bool MakeNodeInfo(DictUnit& node_info, const string& word, double weight, const string& tag) {
    if (!DecodeRunesInString(word, node_info.word)) {
      std::cerr << "Decode " << word << " failed." << std::endl;
      return false;
    }
