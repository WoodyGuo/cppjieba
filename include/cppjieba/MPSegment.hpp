...
class MPSegment : public SegmentTagged {
 public:
  ...
  void Cut(const string& sentence, vector<Word>& words,
 ...
 private:
  void CalcDP(vector<Dag>& dags) const {
    ...
    for (vector<Dag>::reverse_iterator rit = dags.rbegin(); rit != dags.rend(); rit++) {
      ...
      for (auto it = rit->nexts.begin(); it != rit->nexts.end(); it++) {
        nextPos = it->first;
        p = it->second;
        val = 0.0;
  ...
  void CutByDag(RuneStrArray::const_iterator begin, RuneStrArray::const_iterator end,
                const vector<Dag>& dags, vector<WordRange>& words) const {
    size_t i = 0;
    while (i < dags.size()) {
